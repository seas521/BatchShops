 ...
 * 出口商家创建建帐户、子账号以及开店
 *  出口商家创建建帐户、子账号以及开店
 * [针对自主招商商家  开店二级类目多个（规则例：[201001,201009][208007]）或全类目]
 * 
 * @author zhw <br>
 *         Created at 2014年7月15日
 */
public class BatchOpenShopExaMigrationTaskV5 extends ImageMigrationTask {

	public BatchOpenShopExaMigrationTaskV5() throws FileNotFoundException,
			IOException {
		super();
	}

	List<String[]> list = null;
	public static final String EXPORT_FILE = "E:/批量导入/速卖通/开店/batch0715.csv";
	public static final String FILE_PATH = "E:/速卖通开店账号密码备份";// 帐号密码保存路径
	public static final String FILE_NAME = "新建账号密码batch0715";// 文件名
	public static final String ERROR_FILE_NAME = "开店失败商家batch0715";// 开店失败商家文件名
	...
	private void createSellerBrandShop(Connection conn, List<Seller> sellerList)
		...
		String createShopSQL = "INSERT INTO `shop` (`name`,  `seller_id`,`type`,  `trade`,  `operator_id`) VALUES (?,?,?, '2', '156')";
		...
		for (Seller seller : sellerList) {
			try {
				...
				}else if(seller.getCatLev2Id().equals("0")){ //开全部二级类目
					String getCatLv2 = "SELECT id FROM category where level=2";
					PreparedStatement psGetAllCatLv2 = conn
							.prepareStatement(getCatLv2);

					ResultSet Crs = null;
					Crs = psGetAllCatLv2.executeQuery();
					while (Crs.next()) {
						int catId = Crs.getInt("id");
						psShopDeduction.setInt(1, sdid);
						psShopDeduction.setLong(2, catId);
	...
	public static void main(String[] args) throws FileNotFoundException,
		...
		BatchOpenShopExaMigrationTaskV5 task = new BatchOpenShopExaMigrationTaskV5();
		task.run();
		System.exit(0);
	}
}
 ...
 */
public class BatchOpenShopExaMigrationTaskV7 extends ImageMigrationTask {

	public BatchOpenShopExaMigrationTaskV7() throws FileNotFoundException,
			IOException {
		super();
...
//			e.printStackTrace();
	...
	private void createSellerBrandShop(Connection conn, List<Seller> sellerList)
		...
		// create seller
		String createSellerSQL = "INSERT INTO `seller` (`accountName`,  `password`, `companyName`, `companyAddress`, `email`, `contactPerson`, `contactPhone`, `delivery_country_id`,`operating_mode`,`payment_destination`,`business_location`,`bank_location`,`trade`,`city`,`seller_country_id`,review_status) VALUES (?,?,?, ?, ?, ?, ?, '10', '1','1' , '1' , '1' , '2' , '"+getCity()+"','10','3')";
...
//				} else {
				...
				fileList =sellerFileList(seller.getAccountName().substring(4));
				if(fileList.size()!=0){
					for(SellerFile s:fileList){
						if(s.getFile()!=null){
							...
							String errorMessage="账号  " + seller.getAccountName()+" 的"+ SELLER_FILE[s.getType()-1]+"信息上传失败";
...
//				}
			...
			for(int i=0;i<4;i++){
				if(new File(getImg_path()+"/"+sellerName+"/"+SELLER_FILE[i]+".png").exists()){
					...
					try {
						...
						sellImg.setType(i+1);
						String imgKey=tmt.uplodeImg(getImg_path()+"/"+sellerName+"/"+SELLER_FILE[i]+".png");
					...
					try {
						...
						SellerFile sellImg=new SellerFile();
						sellImg.setType(i+1);
						String imgKey=tmt.uplodeImg(getImg_path()+"/"+sellerName+"/"+SELLER_FILE[i]+".JPG");
						sellImg.setFile(imgKey);
						System.out.println(imgKey);
						fileList.add(sellImg);
					} catch (SQLException e) {
	...
	IOException, SQLException {
		BatchOpenShopExaMigrationTaskV7 task=new BatchOpenShopExaMigrationTaskV7();
